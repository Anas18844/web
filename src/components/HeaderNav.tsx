...
export function HeaderNav({ items }: { items: NavItem[] }) {
  ...
  return (
    <>
      {/*
       * Physical CENTRE — links inside a glass capsule, from `xl` up.
       *
       * The capsule used to appear at `md` while quietly hiding /about until
       * `lg` and /parents until `xl`, so those pages had no route from the
       * header at all across a wide band of widths. It now appears only where
       * all six links genuinely fit between the mark and the primary action;
       * everywhere below that, the menu carries the same six.
       */}
      <nav
        aria-label="التنقل الرئيسي"
        className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-1 rounded border border-white/[0.08] bg-white/[0.04] p-1 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.05)] xl:flex"
      >
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                // whitespace-nowrap: with six items the capsule was breaking
                // two-word labels onto a second line and growing to 2 rows.
                'relative inline-flex items-center whitespace-nowrap rounded px-3 py-1.5 text-sm font-bold transition-colors duration-150',
                active
      ...
      <button
        ...
        className="-me-1 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded border border-white/[0.08] bg-white/[0.04] text-ink transition-colors duration-150 hover:border-gold/40 xl:hidden"
      >
      ...
      <div
        id="site-menu"
        className={cn(
          'absolute inset-x-0 top-full border-b border-white/[0.08] bg-navy-deep/[0.98] backdrop-blur-xl xl:hidden',
